    ...
    <div className="lg:w-80 space-y-4">
      ...
      <div className="space-y-4">
        {insights.map((insight, idx) => (
          <div key={idx} className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-slate-900 dark:text-white text-sm">{insight.title}</h3>
              {insight.trend === 'up' && <span className="text-green-500 text-xs">▲</span>}
              {insight.trend === 'down' && <span className="text-red-500 text-xs">▼</span>}
              {insight.trend === 'stable' && <span className="text-slate-400 text-xs">●</span>}
            </div>
